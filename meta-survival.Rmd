...
## Early tested existing sites for now
...
strata_cols <- data.frame(
  strata = unique(meta_surv_df$strata), 
  strata_col = rev(rcartocolor::carto_pal(4, "Vivid")))
surv_plot_df <- meta_surv_df %>%
  left_join(strata_cols, by = 'strata') 

groups <- c("Central", "Peripheral", "Both")
surv_curves <- list()
for (group in groups) {
  # cat(group)
  surv_curves[[group]] <- surv_plot_df %>% 
  filter(strata %in% c("None", group),
         # outcome != "Time to last discharge"
         ) %>% 
  ggplot(aes(
    x = time, y = meta_surv,
    # color = fct_reorder2(strata_col, outcome, meta_surv)
    color = strata_col
  )) +
  geom_line(aes()) +
  # geom_ribbon(aes(ymin = ymin, ymax = ymax), alpha = 0.1, linetype = 2) +
...
cowplot::plot_grid(plotlist = surv_curves, ncol = 1, labels = 'AUTO')
```
