...
```{r}
results <- results[results %in% c("NWU_results", "UPENN_results", "UPITT_results",
                  "VA1_results", "VA2_results", "VA3_results", 
                  "VA4_results", "VA5_results", "MGB_results")]
```
...
medians_df <- combined_other_table_wide %>%
  ...
  pivot_longer(None:Central, names_to = "neuro_type") %>%
...
kbl(tableOne_compiled) %>%
  ...
  pack_rows("Past Medical History", 20, 20) %>%
  pack_rows("Length of Stay", 21, 22) %>%
...
### Top Central ICD Codes

```{r}
datatable(diag_counts %>% filter(pns_cns == "Central") %>% head(5))
```

### Top Peripheral ICD Codes

```{r}
datatable(diag_counts %>% filter(pns_cns == "Peripheral") %>% head(5))
```
...
comorb_table_wide <- bind_rows(comorb_table_list) %>% 
  ...
  distinct(Comorbidity, Comorb_Total, None_Total, CNS_Total, PNS_Total) %>% 
  ungroup()
...
patients_none = as.numeric(gsub( " .*$", "", tableOne_compiled[1,3]))
...
comorb_table_wide_binary <- bind_rows(comorb_table_list) %>% 
  data.frame() %>% 
  group_by(Comorbidity) %>% 
  mutate(Comorb_Total = sum(n_Total, na.rm = TRUE),
         Comorb_Perc = round(Comorb_Total/as.numeric(tableOne_compiled[1,2])*100, 1),
...
datatable(comorb_table_wide_binary)

write.csv(comorb_table_wide_binary, "demo_results/comorbidity_table.csv", row.names = FALSE)
...
```{r}
list_of_comorbs <- comorb_table_wide_binary$Comorbidity

chi_result_list <- list()

for (i in list_of_comorbs) {
  
  comorb <- comorb_table_wide_binary %>% filter(Comorbidity == paste(i))
...
chi_result_table <- chi_result_table %>% 
  ...
  left_join(., comorb_table_wide_binary %>% 
...
write.csv(comorb_table_wide_stratified, "demo_results/comorbidity_table_statified.csv", row.names = FALSE)
```
