...
for (i in results) { 
  tmp <- get(i)
  other_table_list[[i]] <- tmp[[c("first_hosp_results",
                                 "cpns_results",
                                 "other_obfus_table")]] %>% 
    select(site, everything())
}
...
for (i in vars_to_process) {
  ...
  mutate(
    ...
    Central = sub("(\\(.*|\\[.*)", "", Central),
    Both = sub("(\\(.*|\\[.*)", "", Both)) %>% 
  mutate_across.(c(None:Both), as.numeric) %>% 
    as_tibble %>% 
    select(-site, -var) %>%
...
other_table_compiled <- bind_rows(processed_list, .id = "Table 1") %>%
  ...
  mutate(None = paste(None_median, "[", None_min, ",", None_max, "]"),
         Peripheral = paste(Peripheral_median, "[", Peripheral_min, ",", Peripheral_max, "]"),
         Central = paste(Central_median, "[", Central_min, ",", Central_max, "]"),
         Both = paste(Both_median, "[", Both_min, ",", Both_max, "]")) %>%
  mutate(N = NA) %>%
...
p1 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Mean Elixhauser score (SD)") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Mean Elixhauser score (SD)")

p2 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Median Elixhauser score [Min, Max]") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Median Elixhauser score [Min, Max]")

plot_grid(p1, p2)

p3 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Mean length of stay (SD)") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Mean length of stay (SD)")

p4 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Median length of stay [Min, Max]") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Median length of stay [Min, Max]")

plot_grid(p3, p4)


p5 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Mean number of readmissions (SD)") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Mean number of readmissions (SD)")

p6 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Median number of readmissions [Min, Max]") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Median number of readmissions [Min, Max]")

plot_grid(p5, p6)


p7 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Mean time to death (SD)") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Mean time to death (SD) ")



p8 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Median time to death [Min, Max]") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Median time to death [Min, Max]")

plot_grid(p7, p8)

p9 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Mean time to first readmission (SD)") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Mean time to first readmission (SD)")

p10 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Median time to first readmission [Min, Max]") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Median time to first readmission [Min, Max]")

plot_grid(p9, p10)


p11 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Mean time to first readmission (SD)") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
...
p12 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Median time to first readmission [Min, Max]") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
...
p13 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Mean time to severe (SD)") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle("Mean time to severe (SD)")

p14 <- ggplot(other_table_for_figs %>%
         pivot_longer(cols = None:Both,
                      names_to = "type") %>%
         filter(var == "Median time to severe [Min, Max]") %>%
         mutate(type = factor(type, levels = c("None", "Central", "Peripheral", "Both"))),
       aes(y = value, x = type, fill = site)) + 
  geom_bar(stat="identity", position = "dodge", colour="black") + 
   scale_fill_brewer(palette = "Dark2") + 
  ggtitle(" Median time to severe [Min, Max]")
...
row_order <- c("All Patients", "Female", "Male",
...
tableOne_compiled <- tableOne_compiled %>%
  mutate(N = if_else(`Table 1` == "Median length of stay [Min, Max]", 
                     tableOne_compiled %>% 
