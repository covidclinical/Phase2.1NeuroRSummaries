...
kbl(tableOne_compiled) %>%
  ...
  pack_rows("Hospital Course", 22, 24) %>%
...
comorb_table_wide <- bind_rows(comorb_table_list) %>% 
  ...
  distinct(Comorbidity, Comorb_Total, Comorb_Perc, None_Total, None_Perc, CNS_Total, CNS_Perc, PNS_Total, PNS_Perc, Both_Total, Both_Perc) %>% 
  select(Comorbidity, Comorb_Total, Comorb_Perc, None_Total, None_Perc, CNS_Total, CNS_Perc, PNS_Total, PNS_Perc, Both_Total, Both_Perc) %>% 
...
chi_result_table <- chi_result_table %>% 
  ...
  left_join(., comorb_table_wide %>% 
                                select(Comorbidity, None_Perc, Neuro_Perc), by = "Comorbidity") %>% 
  select(Comorbidity, None_Perc, Neuro_Perc, everything()) %>% 
