...
gender_plot <- gender_count %>%
  ...
  geom_text(
    ...
    aes(
      y = site, x = total_count - 22 * sqrt(abs(total_count)),
  ...
  geom_text(
    ...
    aes(
      y = site, x = total_count + 19 * sqrt(abs(total_count)),
      label = abs(total_count)
    ), color = "grey10", size = 3
  ) +
  geom_text(
    data = . %>% filter(site == "APHP", Demo_var_i == "Female"),
    aes(y = length(sorted_sites), x = total_count - 600), fontface = 2,
    label = female, size = 3, color = "white"
  ) +
  geom_text(
    data = . %>% filter(site == "APHP", Demo_var_i == "Male"),
    aes(y = length(sorted_sites), x = total_count + 600), fontface = 2,
...
age_plot <- age_sum %>%
  ggplot(aes(
    y = fct_reorder(site, freq * (-1 + 2 * (Demo_var_i == "
80+"))),
...
race_plot <- race_sum %>%
  ggplot(aes(
    y = fct_reorder(site, freq, .fun = max),
    x = freq,
...
cowplot::plot_grid(gender_plot, age_plot, race_plot,
  ncol = 1,
