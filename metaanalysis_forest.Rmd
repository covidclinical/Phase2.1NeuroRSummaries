...
# remove sites with < 5 in the 'neuro_chr' type of interest
...
for (i in sorted_sites) {
  ...
  diag_table_list[[i]] <- tmp_diag %>%
    filter(variable %in% c("sex.male",
                         "sex.female")) %>% 
                         #"readmitted.TRUE",
                         #"Severity.Severe",
                         #"Survival.Deceased")) %>% 
    select(site, contains("n_var")) %>%
    pivot_longer(cols = n_var_Peripheral:n_var_Both,
                 names_to = "diagnosis",
                 values_to = "count") %>% 
    group_by(diagnosis, site) %>% 
    mutate(sum = sum(count, na.rm = TRUE)) %>%
    slice(1L) %>%
    ungroup()
}
...
# sites to exclude
if (nrow(diag_table_wide %>%
  filter(sum < 5)) > 0 & neuro_chr_to_analyze == "neuro_postBoth") {
  
  exclude <- diag_table_wide %>%
  filter(sum < 5) %>%
    select(site)
  
  exclude <- exclude$site
  
  exclude <- paste(exclude, "results", sep = "_")
  sorted_sites <- sorted_sites[!(sorted_sites %in% exclude)]
}

if (nrow(diag_table_wide %>%
  filter(sum < 5)) > 0 & neuro_chr_to_analyze == "neuro_postPeripheral") {
...
## need to remove specific sites from contributing to meta-analysis based on outcomes < 5
...
combined_results <- combined_results %>%
  ...
  mutate_at(vars(TE:upper.predict, -pval.random), ~ if_else(analysis == "readmit_reg_elix", exp(.),
...
write.csv(results_table, file = paste0("meta_tables/meta_tables", neuro_chr_to_analyze, ".csv"), 
          row.names = FALSE, )
```
...
min_est = min(length_stay_results$CI.Low, na.rm = TRUE)-2
max_est = max(length_stay_results$CI.High, na.rm = TRUE)+2
...
forester(left_side_data = length_stay_results[1],
         ...
         xlim = c(min_est, max_est),
         xbreaks = c(min_est, 0, max_est),
         point_sizes = sizes,
         point_shapes = shapes,
...
min_est = min(severe_results$CI.Low, na.rm = TRUE)-2
max_est = max(severe_results$CI.High, na.rm = TRUE)+2
...
min_est = min(death_results$CI.Low, na.rm = TRUE)-2
max_est = max(death_results$CI.High, na.rm = TRUE)+2
...
min_est = min(readmit_results$CI.Low, na.rm = TRUE)-2
max_est = max(readmit_results$CI.High, na.rm = TRUE)+2
...
min_est = min(severe_time_results$CI.Low, na.rm = TRUE)-2
max_est = max(severe_time_results$CI.High, na.rm = TRUE)+2
...
min_est = min(death_time_results$CI.Low, na.rm = TRUE)-2
max_est = max(death_time_results$CI.High, na.rm = TRUE)+2
...
min_est = min(readmit_time_results$CI.Low, na.rm = TRUE)-2
max_est = max(readmit_time_results$CI.High, na.rm = TRUE)+2
...
min_est = min(readmit_n_results$CI.Low, na.rm = TRUE)-0.5
max_est = max(readmit_n_results$CI.High, na.rm = TRUE)+0.5
